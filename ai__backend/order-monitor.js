      ...
      try {
        ...
        if (metadata.coverImage) {
          try {
            ...
            // Cover full width and center vertically
            try {
              const imageMetadata = await sharp(coverBuffer).metadata();
              const imageAspectRatio = imageMetadata.width / imageMetadata.height;
              
              let imageWidth = A4_WIDTH;
              let imageHeight = A4_WIDTH / imageAspectRatio;
              let y = 0;
              
              // Center vertically if image is shorter than page
              if (imageHeight < A4_HEIGHT) {
                y = (A4_HEIGHT - imageHeight) / 2;
              } else {
                // If taller, fit to height and center horizontally
                imageHeight = A4_HEIGHT;
                imageWidth = A4_HEIGHT * imageAspectRatio;
              }
              
              const x = (A4_WIDTH - imageWidth) / 2; // Center horizontally
              doc.image(coverBuffer, x, y, {
                width: imageWidth,
                height: imageHeight
              });
            } catch (error) {
              // Fallback: cover full width
              doc.image(coverBuffer, 0, 0, {
                width: A4_WIDTH
              });
            }
            
            console.log("✅ Cover image added to PDF (full width, centered)");
          } catch (error) {
        ...
        if (personalizedBook.pages && personalizedBook.pages.length > 0) {
          ...
          const sortedPages = personalizedBook.pages.sort((a, b) => a.pageNumber - b.pageNumber);
          ...
          for (const page of sortedPages) {
            ...
            if (page.processedImage) {
              pageBuffer = Buffer.from(page.processedImage, "base64");
              isProcessed = true;
            } else if (page.pageImage) {
              pageBuffer = Buffer.from(page.pageImage, "base64");
              isProcessed = false;
            } else {
            ...
            try {
              ...
              try {
                ...
                const x = (A4_WIDTH - imageWidth) / 2; // Center horizontally
                doc.image(pageBuffer, x, y, {
                  width: imageWidth,
                  height: imageHeight
                });
                
                if (isProcessed) {
                  console.log(`✅ Page ${page.pageNumber} added to PDF (full width, centered)`);
                } else {
                  console.log(`⚠️  Page ${page.pageNumber} added (using original, processing failed)`);
                }
              } catch (error) {
                // Fallback: cover full width
                ...
                console.log(`⚠️  Page ${page.pageNumber} added (fallback mode)`);
              }
            } catch (error) {
              console.error(
