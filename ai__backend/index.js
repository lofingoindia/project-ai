...
// Initialize the complete book personalization service
...
app.post('/process-complete-book', async (req, res) => {
    try {
        const { 
            bookPages, // Array of base64 page images
            childImage, 
            childName, 
            bookTitle,
            processingOptions = {}
        ...
        console.log(`📚 Processing complete book: ${bookTitle}`);
        ...
        if (!bookPages || !Array.isArray(bookPages) || bookPages.length === 0) {
            return res.status(400).json({ 
                error: 'bookPages array is required and must not be empty' 
            });
        }
        
        if (!childImage) {
            return res.status(400).json({ 
                error: 'childImage is required' 
            });
        }
        ...
        if (result.success) {
            return res.json({
                ...
                bookAnalysis: result.bookAnalysis,
        ...
        return res.status(500).json({ 
            ...
            message: 'Internal server error during book processing'
        });
    }
});

// Book analysis endpoint (for preview/analysis only)
app.post('/analyze-book', async (req, res) => {
    try {
        ...
        const bookAnalysis = await bookPersonalizationService.analyzeCompleteBook(bookPages);
        ...
        return res.status(500).json({ 
            ...
            message: 'Internal server error during book analysis'
        });
    }
});
